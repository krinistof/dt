...
**Note:** The same way the previous prototype helped explore the challenge of creating a robust voting system but failed to be maintainable, this version of the project is deprecated and is no longer being actively developed. The main development effort has shifted to the React/gRPC version of the application in the `frontend` and `backend` directories.
